...
```bash
# Create directories
mkdir -p ~/.claude/skills ~/.claude/sdlc/prompts

# Copy skills (the commands you'll run)
cp skills/*.md ~/.claude/skills/

# Copy prompts (the stage-specific AI instructions)
cp prompts/*.md ~/.claude/sdlc/prompts/

# Copy conventions (the system rules)
cp conventions.md ~/.claude/sdlc/conventions.md
```
...
```bash
# Default mode — every 15 minutes (~$48/day)
*/15 * * * * /usr/local/bin/claude -p "Run /sdlc-orchestrate" >> /tmp/sdlc-orchestrate.log 2>&1

# Sprint mode — every 5 minutes (~$144/day, use during active dev only)
...
```
Warning at $5 → change to your preferred warning threshold
Hard stop at $15 → change to your preferred hard stop
```
...
- Review the [Gap Registry](gap-registry.md) to understand the 23 failure modes AgentFlow addresses
